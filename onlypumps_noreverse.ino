...
void setup() {
  ...
  pinMode(driverOut1, OUTPUT); // Add this line to set driverOut1 as output
  pinMode(driverOut2, OUTPUT); // Add this line to set driverOut2 as output
  ...
  if (firstBracketIndex != -1 && secondBracketIndex != -1) {
    ...
    if (spaceIndex != -1) {
      ...
      if (motorType == "PUMPMOTOR_OPERATION") {
        ...
        digitalWrite(driverOut2, LOW); // Turn off driverOut2
        isReverse = false;
      } else if (motorType == "REVERSE_PUMPMOTOR_OPERATION") {
        digitalWrite(driverOut1, LOW); // Turn off driverOut1
