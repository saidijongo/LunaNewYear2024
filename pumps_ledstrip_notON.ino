...
int maxRunTime = 0; // Find the maximum runTime for param2 values.
...
void setup()
{
  ...
  digitalWrite(driverOut1, LOW); // driverOut1 is initially low
  digitalWrite(driverOut2, LOW); // driverOut2 is initially low
...
void runPumps(int pumpNumber, int runTime)
{
  ...
  // Turn on the LED strip with the maximum run time of the pumps
...
void blinkLEDs() {
  const int blinkDuration = 10000;  // 20 seconds
        ...
        if (nextBracketIndex != -1 && endIndex != -1)
        {
          ...
          if (commaIndex != -1)
          {
            ...
            if (currentMotorType == "PUMPMOTOR_OPERATION" || currentMotorType == "REVERSE_PUMPMOTOR_OPERATION")
            {
              ...
              // Turn on the LED strip when pumps start running
              ledStrip(defaultSpeed, maxRunTime);
            }
...
void loop()
{
  ...
  else if (motorState == RUNNING_PUMPS)
  {
    ...
    if (allPumpsOff)
    {
      ...
      delay(1000);
      // Turn off the LED strip after the maxRunTime
      //ledStrip(defaultSpeed, maxRunTime);
